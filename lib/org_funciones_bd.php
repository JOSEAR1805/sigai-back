...
function datosperfil($json_data){
	...
	if ($Numfilas1 > 0){				
		while ($fila1 = pg_fetch_array($Consulta1)) {
			...
			$mnom = utf8_decode($fila1['nombre_menu']);	
			$desc = ($fila1['url']);
	...
	ind2.nb_vicepresidencia,ind2.id_vice_presidencia,ind2.id_gerencia,
	CASE WHEN ip.id_indica IS NULL THEN 0 ELSE 1 END AS activo 
	FROM
	(SELECT id_indicador, nb_indicador,gc.id_gerencia, gc.nb_gerencia, 
	vp.nb_vicepresidencia,vp.id_vice_presidencia
	FROM cantv.indicadores as ind, cantv.gerencia as gc, cantv.vice_presidencia as vp
	WHERE ind.id_gerencia=gc.id_gerencia
	AND gc.id_vice_presidencia=vp.id_vice_presidencia) as ind2
	LEFT JOIN
	(SELECT id_indica FROM cantv.indicaxperfil
	WHERE id_perfil='.$idPerf.') as ip
	ON ind2.id_indicador=ip.id_indica
	ORDER BY ind2.nb_vicepresidencia, ind2.nb_gerencia, ind2.nb_indicador';
	...
	if ($Numfilas3 > 0){				
		while ($fila3 = pg_fetch_array($Consulta3)) {
			...
			$idvp = intval($fila3['id_vice_presidencia']);
			$idgc = intval($fila3['id_gerencia']);		
			$nind = utf8_decode($fila3['nb_indicador']);	
			$gcia = utf8_decode($fila3['nb_gerencia']);				
			$nbvp = utf8_decode($fila3['nb_vicepresidencia']);					
			$iact = filter_var(($fila3['activo']), FILTER_VALIDATE_BOOLEAN);							

			$ArrayData3[] = array('idIndicador'=>$idin,'idUnidadOrganizativa'=>$idvp,'unidadOrganizativa'=>$nbvp,'idGerencia'=>$idgc,'gerencia'=>$gcia,'nombreIndicador'=>$nind,'activo'=>$iact); 					
		}	
		$tablas = array('gerencia','vice_presidencia');	
		$i=4;
	...
	"menu"=>$ArrayData1, "vistas"=>$ArrayData2, "indicadores"=>$ArrayData3, $tabla4=>$ArrayData4, 'unidadOrganizativa'=>$ArrayData5);
	return json_encode($json);
}
...
function ficha($json_data){		
	...
	//Estas son las tablas con Id por pasar ordenadas en un arreglo desde 1 (ArryData1) hasta 8 (ArrayData8)
	...
	'categoria_indicador','vice_presidencia','tipo_cobertura');
	...
	//Solo Para la tabla unidades_medicion ( Normal y Financiera ) -->
	foreach ($ArrayData2 as $clave) {
		
		foreach ($clave as $llave => $valor) {
			if ($llave='financiera' && $valor=='s') {
				unset($clave["financiera"]);
				$ArrayData21[] = $clave;
			}	
	...
	//Las fiancieras tienen el campo financiera='s' <--

	//Fin de armar tablas soporte con camelCase								

	$json = array("estatus"=>$Estatus, "mensaje"=>$Mensaje, "ficha"=>$ArrayData, $tabla1=>$ArrayData1, $tabla2=>$ArrayData20, 
	$tabla3=>$ArrayData3, $tabla4=>$ArrayData4, $tabla5=>$ArrayData5, $tabla6=>$ArrayData6, $tabla7=>$ArrayData7,
	$tabla8=>$ArrayData8,$tabla9=>$ArrayData9,'unidadMedidaFinanciera'=>$ArrayData21,$tabla10=>$ArrayData10);	
...
function listamenu($json_data){
	...
	FROM cantv.menu as mm, cantv.menu_perfil as mp,
	cantv."Usuarios" as us,cantv.paginas as pg,cantv.iconos ic
	WHERE mm.id_menu=mp.id_menu
	AND mp.id_perfil=us.id_perfil
		...
		foreach ($columnas as $indice){				
			foreach ($indice as $key => $valor){
				if(strpos($key,'_') == true){
					...
					$indice[$a] = ctype_digit($indice[$key]) ? intval($indice[$key]) : utf8_decode($indice[$key]);
					//$indice[$a] = $valor;
					unset($indice[$key]);	
				}else{
					$a = $key;
