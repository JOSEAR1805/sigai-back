...
function guardarperfil($json_data){
	...
	$idperf = $json_data->idPerfilCon;		
	$nombre = utf8_decode($json_data->nombre);	

	if (isset($json_data->descripcion)) {	
		$descri = utf8_decode($json_data->descripcion);
	}else{
		$descri = "";
	}
	...
	if ($Numfilas > 0){
		...
		# inicia la inclusion del nuev perfil
		$query1='INSERT INTO cantv."Perfiles"
...
function vistas($json_data){
    ...
    AND pg.activo = true
    AND us.id_usuario ='. $idUser .' AND url='.chr(39).$vista.chr(39);	
