...
function datosperfil($json_data){
	...
	//Pendiente por definir el proceso de las vistas
	...
	$dbConn= new AccesoDB;
	//Trae los datos del perfil y menu asociados
	$query='SELECT m1.id_menu, m1.nombre_menu, m1.id_perfil, m1.orden, m1.padre, m1.id_icono,
	pe.nombre_perfil, pe.descripcion
	FROM cantv.menu as m1, cantv."Usuarios" as us, cantv."Perfiles" as pe
	WHERE m1.id_perfil=us.id_perfil
	AND us.id_perfil=pe.id_perfil	
	...
	if ($Numfilas>0){				
		while ($fila = pg_fetch_array($Consulta)) {
			...
			$imen = intval($fila['id_menu']);	
			$mnom = utf8_decode($fila['nombre_menu']);	
			$mord = intval($fila['orden']);	
			$mpad = intval($fila['padre']);	
			$ArrayData1[] = array('idMenu'=>$imen,'nombreMenu'=>$mnom,'orden'=>$mord,'padre'=>$mpad); 
		}
		//Para evitar datos repetidos en el menu
		$ArrayData1 = array_unico($ArrayData1, 'idMenu');		

		//Buscando TODAS las paginas registradas
		$query1 = 'SELECT pg.id_pagina, pg.nombre_pagina, pg.url, pg.activo, pg.descripcion
		FROM cantv.paginas as pg';
		...
		if ($Numfilas1>0){				
			while ($fila1 = pg_fetch_array($Consulta1)) {
				$ipag = intval($fila1['id_pagina']);	
				$pnom = utf8_decode($fila1['nombre_pagina']);	
				$pdes = utf8_decode($fila1['descripcion']);	
				$purl = utf8_decode($fila1['url']);	
				$pact = ($fila1['activo']);		

				$query2 = 'SELECT id_perfil 
				FROM cantv.pagina_perfil
				WHERE id_perfil='.$iper. ' AND id_pagina='.$ipag;

				$Consulta2=$dbConn->db_Consultar($query2);
				$Numfilas2=$dbConn->db_Num_Rows($Consulta2);
				//Saber si tiene o NO permiso el perfil de consulta sobre la pagina
				if($Numfilas2>0){
					//Si lo tiene tomo el perfil
					$iper2 = $iper;
				}else{
					//No lo tiene coloco cero (0) y completo los datos de la pagina
					$iper2 = 0;
				}
				$ArrayData2[] = array('idPagina'=>$ipag,'nombrePagina'=>$pnom,'descripcion'=>$pdes,'url'=>$purl,'activo'=>$pact,'idPerfil'=>$iper2); 					
			}
		}	
		//Verificacion de posibles duplicados
		$ArrayData2 = array_unico($ArrayData2, 'idPagina');
		$Estatus=1;
		$Mensaje="Datos Perfil";		
	}else{
		$Estatus=2;
		$Mensaje="Sin Datos del perfil";		
		$ArrayData1=array('idMenu'=>0,'nombreMenu'=>'','orden'=>0,'padre'=>0); 
		$ArrayData2=array('idPagina'=>0,'nombrePagina'=>'','descripcion'=>'','url'=>'','activo'=>'','idPerfil'=>0); 					
		$iper=0;
		$nper='';
		$desc='';
	}	

	$json = array("estatus"=>$Estatus, "mensaje"=>$Mensaje, "idPerfil"=>$iper, "nombrePerfil"=>$nper, "descripcionPerfil"=>$desc, 
	"menuPerfil"=>$ArrayData1, "paginas"=>$ArrayData2);
	return json_encode($json);
}
...
function listausuarios($json_data){
	...
	if ($Numfilas>0){				
		...
		while ($fila = pg_fetch_array($Consulta)) {
			...
			$nus = utf8_decode($fila['nb_usuario']);	
			...
			if(!in_array($idu,$ArrayData4) && $idu > 0){
				...
				$ArrayData3[] = array('idVicePresidencia'=>$idv,'idGerencia'=>$idg,'idUsuario'=>$idu,'nombreUsuario'=>$nus);	
			}            
		}
